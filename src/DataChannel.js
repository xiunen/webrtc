...
class App extends PureComponent {
  ...
  sendMsg = () => {
    ...
    const msgItem = { name, message, timestamp: Date.now() }
    const data = JSON.stringify(msgItem)
    ...
    return (
      <div style={{ height: 380, padding: 10 }}>
        <ul id='msgs'>
          {
            messages.map(item => (
              <li key={item.name + '-' + item.timestamp} style={item.name == name ? { textAlign: 'right' } : {}} >
                <div>
                  <span>{item.name}</span>
                  <time style={{
                    fontSize: 12,
                    marginLeft: 5,
                    color: '#888'
                  }}>{(new Date(item.timestamp)).toString()}</time>
                </div>
                <div style={{ backgroundColor: '#eee', borderRadius: 10, padding: '5px 10px', display: 'inline-block' }}>{item.message}</div>
              </li>
            ))
          }
        </ul>
